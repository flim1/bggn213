...
In this data-set we have 212 cancer and 357 non cancer patients.
...
To find out how many there are I can call `length()` on the result of
`grep()`
...
Looks like we need to use `scale=TRUE` here as our data are all over the
shop… (because the sd of the means are very different, hence we could
mislead data analysis for thinking that those with large sd are more
important)
...
Minimum number of principal components required to describe at least 90%
of variability is 7 PCs
